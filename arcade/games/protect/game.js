...
function updateSpecialWeapons(currentTime, width, height) {
    ...
    if (stats.iceSlowLevel > 0) {
        const slowPercent = 0.3 + stats.iceSlowLevel * 0.1;
        enemies.forEach(e => {
            if (!e.iceSlowed) {
    ...
    if (stats.haloLevel > 0) {
        ...
        const haloDamage = 0.05 * stats.haloLevel; // Increased
    ...
    // FIREBALLS - auto aim, slower but 2x damage
    if (stats.fireballCount > 0 && currentTime - lastFireballTime > 3000) { // 3s cooldown (slower)
        ...
        if (nearest) {
            for (let i = 0; i < stats.fireballCount; i++) {
                ...
                fireballs.push({
                    x: player.x,
                    y: player.y,
                    vx: Math.cos(angle) * 3.5, // Slower
                    vy: Math.sin(angle) * 3.5,
                    damage: 2.0 + stats.fireballCount * 0.3, // 2x base damage
                    life: 80
                });
            }
    ...
    if (stats.bladeCount > 0) {
        ...
        const bladeSpeed = 0.06;
        const bladeDamage = 0.5 + stats.bladeCount * 0.15;
